 ...
 * ready-to-paste <CloudinaryPicture> snippet (intrinsic width/height pulled
 * from the upload response) to stdout.
 *
 * Usage:
 *   pnpm cloudinary:breakpoints -- public/assets/blog/cover.jpg
 *   pnpm cloudinary:breakpoints -- public/assets/blog/cover.jpg --sizes="100vw"
 *
 ...
 * or type a custom sizes string for simple responsive mode. --devices="..."
 * or --sizes="..." skips the prompt. In a non-interactive shell all devices
 * are used. These only affect the printed snippet, not the upload.
 *
 * The Cloudinary public ID is derived from the file path by dropping the
 * leading "public/" or "src/" segment and the file extension, e.g.
 *   public/assets/blog/cover.jpg -> assets/blog/cover
 *   src/assets/images/photo.jpg  -> assets/images/photo
 *
 * Credentials are read from .env via Node's --env-file-if-exists flag (see
 * package.json), or from the surrounding process environment. Only this
 * upload script needs the API key/secret — the build does not.
 */
...
/** Compact `devices` string for the <CloudinaryPicture> prop: "minWidth|vw|aspectRatio,...". */
function buildDevicesString(selected) {
	return selected
		.map((d) => `${d.minWidth}|${d.vw}|${d.aspectRatio}`)
		.join(",");
}
...
function derivePublicId(filePath) {
	...
	let withoutPrefix = rel;
	if (rel.startsWith("public/")) withoutPrefix = rel.slice(7);
	else if (rel.startsWith("src/")) withoutPrefix = rel.slice(4);
	const ext = extname(withoutPrefix);
	return ext ? withoutPrefix.slice(0, -ext.length) : withoutPrefix;
}
...
async function main() {
	...
	if (!filePath) {
		console.error(
			'Usage: pnpm cloudinary:breakpoints -- <path-to-image> [--devices="..."] [--sizes="..."]',
		);
		process.exit(1);
	}
	...
	console.log(
		`Wrote ${sorted.length} breakpoints for "${publicId}" to ${relative(
			ROOT,
			BREAKPOINTS_FILE,
		)}`,
	);
	...
	// Intrinsic dimensions come from the upload response. CloudinaryPicture
	// reads src/alt/width/height/breakpoints plus either `devices` (art
	// direction: per-device crops via <source media>) or `sizes` (simple
	// responsive). It rejects an empty alt — replace the TODO before publishing.
	...
	console.log("\nPaste this into an .astro/.mdx file (replace the alt text).");
	console.log("In .astro files add: import CloudinaryPicture from");
	console.log('  "@/components/ui/CloudinaryPicture.astro";');
	console.log("In markdown, plain ![alt](/assets/...) images are rewritten");
	console.log("automatically and never need this component.\n");
	console.log(snippet);
	console.log("");
}
